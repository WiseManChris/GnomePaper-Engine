...
**Version 1.1.9** · by [WiseManChris](https://github.com/WiseManChris)
...
[![Version 1.1.9](https://img.shields.io/badge/version-1.1.9-brightgreen.svg)](https://github.com/WiseManChris/GnomePaper-Engine/releases)
...
### 1.1.9
- **Steam QR Login Audience Fix**: The QR authentication was previously missing the `website_id: Client` parameter, causing Steam to return tokens meant for Web APIs instead of the Steam Client. This caused `EResult.AccessDenied` when GnomePaper Engine attempted to use the token to download items. Tokens are now correctly minted for the `Client` audience.

### 1.1.8
- **Steam QR Login Fix**: Fixed a token rejection (`EResult.AccessDenied`) caused by a mismatch in OS type during QR token generation vs client login. Tokens are now correctly minted for Windows 10 to match the client payload.

### 1.1.7
- **Steam Guard Routing Fix**: Fixed an issue where entering a Steam Guard code during a native account linking session would incorrectly route the download attempt to SteamCMD, causing linking to fail.
