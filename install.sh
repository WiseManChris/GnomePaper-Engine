...
install_app() {
  ...
  # Seamless Steam: QR auth + CDN downloads (not SteamCMD)
  pip install -U 'steam[client]' gevent protobuf qrcode pillow requests >/dev/null || \
    pip install -U 'steam[client]' gevent qrcode pillow requests >/dev/null || true
